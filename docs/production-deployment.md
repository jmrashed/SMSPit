# Production deployment guide (Day 97)
...
2. `docker compose up -d --build` (or `docker compose pull` once images are published per Day 96, then `docker compose up -d` without `--build`).
...
1. Publish images first (Day 96) — the chart's default `image.repository` values now point at `ghcr.io/jmrashed/smspit-*` (fixed in this change; they previously pointed at unpublished `smspit/*` names, which would have silently failed to pull).
...
- `worker` consumes Redis Streams via a consumer group (`worker/internal/consumer/consumer.go`, Day 78) — multiple replicas is safe, since each replica in the same group gets a distinct share of the stream rather than reprocessing the same messages.
- The gateway's per-org rate limit (`RATE_LIMIT_PER_MINUTE`, Day 86) is **per gateway instance**, not cluster-wide — it's an in-memory fixed-window counter (see [docs/multi-tenancy.md](multi-tenancy.md#hardening-audit-day-86)). Running N gateway replicas behind a load balancer effectively multiplies the real per-org quota by N, since each instance tracks its own counts. Fine at small N; if you scale the gateway significantly, move the limiter to a shared store (Redis) instead of assuming today's in-memory version enforces a hard global cap.
- Bump `resources.requests`/`resources.limits` per service in `values.yaml` based on your own load-testing baseline (Day 88's numbers were measured on a single dev laptop with no Docker, not representative of real infrastructure sizing — re-run `scripts/load-test/` against your actual target environment before picking numbers).
...
| `RATE_LIMIT_PER_MINUTE` | gateway | Per-org request quota (Day 86); see the scaling caveat above |
| `OTEL_EXPORTER_OTLP_ENDPOINT` | gateway, sms-service, auth-service, ai-service, worker | OTLP collector endpoint (Day 83); leave unset to disable tracing |
...
| `DASHBOARD_API_KEY` | dashboard | API key the dashboard container authenticates with (Day 35+) |
| `AI_SERVICE_URL` | sms-service, worker | Internal URL to ai-service; unset degrades OTP/spam/classification to "not detected" (Day 68) rather than failing captures |

Full list with defaults: [.env.example](../.env.example) (Compose) and [deployments/helm/smspit/values.yaml](../deployments/helm/smspit/values.yaml) (Helm).
